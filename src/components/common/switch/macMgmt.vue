    ...
    <div>
        ...
        <div class="query-select">
            ...
            <div v-if="choose_macaddr" class="query-frame">
                ...
                <input
                    ...
                    v-model="macaddr"
                    :style="{ 'border-color' : check_mac.test(macaddr) || macaddr === '' ? '' : 'red' }"
                />
                ...
                <input
                    ...
                    placeholder="ex: 00:00:00:00:00:00"
                    v-model="macmask"
                    :style="{ 'border-color' : check_mac.test(macmask) || macmask === '' ? '' : 'red' }"
                />
        ...
        <ul class="mac-table" v-if="this.mac_table[0]">
            <li>
                ...
                <span
                    v-for="(item,key) in mac_table[0]"
                    :key="key"
                    v-if="key !== 'link_aggregation'"
                >
                    {{ lanMap[key] }}
                    <i
                        class="arrow-sort-up"
            ...
            <li v-for="(item,key) in mac_table" :key="key">
                ...
                <span>{{ item.mac_type === 0 ? lanMap['dynamic'] : item.mac_type === 1 ? lanMap['static'] : lanMap['blackhole'] }}</span>
                <span>
            ...
            <li v-if="pagination.page > 1" class="paginations">
                <ul class="pagination rt">
                    <li
                        :class="pagination.index > 1 ? '' : 'disabled'"
                        @click="changeIndex(1)"
                    >&lt;&lt;</li>
                    <li
                        :class="pagination.index > 1 ? '' : 'disabled'"
                        @click="changeIndex(pagination.index-1)"
                    >&lt;</li>
                    <li
                        v-if="pagination.index>2"
                        @click="changeIndex(pagination.index-2)"
                    >{{ pagination.index-2 }}</li>
                    <li
                    ...
                    <li
                        class="actived"
                        @click="changeIndex(pagination.index)"
                    >{{ pagination.index }}</li>
                    <li
                        v-if="pagination.page-1 >= pagination.index"
                        @click="changeIndex(pagination.index+1)"
                    >{{ pagination.index+1 }}</li>
                    <li
                        v-if="pagination.page-2 >= pagination.index"
                        @click="changeIndex(pagination.index+2)"
                    >{{ pagination.index+2 }}</li>
                    <li
                        :class="pagination.page-1 >= pagination.index ? '' : 'disabled'"
                        @click="changeIndex(pagination.index+1)"
                    >&gt;</li>
                    <li
                        :class="pagination.page !== pagination.index ? '' : 'disabled'"
                        @click="changeIndex(pagination.page)"
                    >&gt;&gt;</li>
                </ul>
            </li>
        </ul>
            ...
            <div class="content">
                ...
                <div class="add-mac-item">
                    ...
                    <input
                        ...
                        :style="{ 'border-color' : this.check_mac.test(this.add_param.macaddr) || !this.add_param.macaddr ? '' : 'red' }"
                    />
                ...
                <div class="add-mac-item place-holder">
                    ...
                    <input
                        ...
                        :style="{ 'border-color' : !isNaN(this.add_param.vlan_id) ? '' : 'red' }"
                    />
        ...
        <nms-dialog :visible.sync="dialogVisible" width="650px">
            ...
            <div slot="footer">
                <nms-button @click="submitMacAuth">{{ lanMap['apply'] }}</nms-button>
...
import { parsePort, getPortName } from "@/utils/common";
export default {
    name: "macMgmt",
    components: { loading },
    computed: {
        ...
        if (this.change_url.beta === "test") {
            this.$http.get(this.change_url.mactab).then((res) => {
                ...
                this.pagination.page = Math.ceil(
                    this.tab.length / this.pagination.display
                );
                this.getPage();
            });
        } else {
    ...
    methods: {
        ...
        getData() {
            ...
            this.$http
                .post("/switch_mac?form=table", post_param)
                .then((res) => {
                    if (res.data.code === 1) {
                        if (!res.data.data) {
        ...
        query_macaddr() {
            ...
            if (
                this.macaddr === "ff:ff:ff:ff:ff:ff" ||
                this.macaddr === "00:00:00:00:00:00" ||
                this.macaddr === "01:00:5e:00:00:00"
            ) {
                this.$message({
                    type: "error",
                    text: this.lanMap["param_error"],
