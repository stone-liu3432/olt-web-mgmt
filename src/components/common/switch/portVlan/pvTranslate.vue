<template>
    <div class="port-vlan-translate">
        ...
        <div
            class="pv-translate-item"
            v-for="(item,key) in pvData.data"
            :key="key"
            v-if="pvData.data && pvData.data.length > 0"
        >
...
export default {
    ...
    methods: {
        ...
        submit_pv_translate() {
            if (
                this.pv_old_vlan === this.cache_d.old_vlan &&
        ...
        open_del_translate(item) {
            this.$confirm(this.lanMap["if_sure"])
                .then(_ => {
                    this.del_pv_trans(item.old_vlan);
                })
                .catch(_ => {});
        },
        //  确认框内的操作按钮
        del_pv_trans(old_vlan) {
            const post_params = {
                method: "delete",
                param: {
                    port_id: this.$parent.$data.portid,
