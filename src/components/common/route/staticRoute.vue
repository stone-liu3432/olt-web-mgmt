<template>
    <div>
        <page-header type="none">
            <template slot="title">{{ lanMap['route_cfg'] }}</template>
        </page-header>
        <!-- <div v-if="default_route.data" style="margin: 20px 0;">
            ...
            <a
                href="javascript:void(0);"
                @click="openDelCfm"
                v-if="default_route.data.gateway !== '0.0.0.0'"
                style="margin-left: 30px;"
            >{{ lanMap['delete'] }}</a>
        </div>
        <hr />-->
        ...
        <div v-if="status">
            <nms-table :rows="static_router" border>
                <nms-table-column :label="`${lanMap['dst_ipaddr']}/${ lanMap['mask']}`">
                    <template slot-scope="rows">{{ `${rows.ipaddress}/${rows.mask_num}` }}</template>
                </nms-table-column>
                <nms-table-column prop="gateway" :label="lanMap['next_hop']"></nms-table-column>
                <nms-table-column :label="lanMap['interface']">
                    <template
                        slot-scope="rows"
                    >{{ rows.interface ? `Vlanif${rows.interface}` : ' * ' }}</template>
                </nms-table-column>
                <nms-table-column prop="protocol" :label="lanMap['protocol']">
                    <template slot-scope="rows">{{ protocols[rows.protocol] }}</template>
                </nms-table-column>
                ...
                <nms-table-column prop="status" :label="lanMap['status']">
                    <template slot-scope="rows">{{ rows.status === 1 ? 'Reachale' : 'Unreachale' }}</template>
                </nms-table-column>
                <nms-table-column :label="lanMap['config']">
                    <template slot-scope="rows">
                        <a
                            href="javascript: void(0);"
                            class="btn-text"
                            @click="openDeleteModal(rows)"
                            v-if="rows.protocol !== 1"
                        >{{ lanMap['delete'] }}</a>
                    </template>
                </nms-table-column>
            </nms-table>
        ...
        <div class="modal-dialog" v-if="!!setModal">
            ...
            <div>
                <div class="set-route-item">
                    ...
                    <div>
                        <a href="javascript:void(0);" @click="setStitacRouter">{{ lanMap['apply'] }}</a>
        ...
        <div class="modal-dialog" v-if="setDefRoute">
            ...
            <div style="height: 160px;">
                <div class="set-route-item">
                    ...
                    <div>
                        <a href="javascript:void(0);" @click="setDef">{{ lanMap['apply'] }}</a>
                        <a href="javascript:void(0);" @click="closeModal">{{ lanMap['cancel'] }}</a>
                    </div>
                </div>
                <div class="close" @click="closeModal"></div>
...
export default {
    ...
    computed: {
        ...
        //         return this.default_route.data.gateway === "0.0.0.0"
        //             ? " -- "
        //             : this.default_route.data.gateway;
        //     }
        //     return "";
        // }
    },
    ...
    methods: {
        ...
        offRoute() {
            if (this.status) {
                this.$confirm(this.lanMap["static_route_tips"])
                    .then(_ => {
        ...
        changeRouteState() {
            if (!this.interval) {
                ...
                this.$http
                    .post("/switch_route?form=admin", post_data)
                    .then(res => {
                        if (res.data.code === 1) {
                            this.$message({
                                type: res.data.type,
                            ...
                            if (
                                res.data.message.indexOf(
                                    "please disable dhcp relay first"
                                ) > -1
                            ) {
                                this.$confirm(this.lanMap["route_close_tips"])
                                    .then(_ => {
                                        this.disableDhcpRelay(
        ...
        getData() {
            this.static_router = [];
            this.$http
                .get(this.change_url.get_static_router)
                .then(res => {
                    if (res.data.code === 1) {
        ...
        setStitacRouter() {
            ...
            this.$http
                .post("/switch_route?form=static_route", post_data)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        disableDhcpRelay(msg) {
            ...
            this.$http
                .post("/switch_dhcp?form=relay_admin", data)
                .then(res => {
                    if (res.data.code === 1) {
                        this.interval = false;
