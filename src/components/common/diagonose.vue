<template>
    <div class="diagonose">
        ...
        <div class="diagonose-item">
            ...
            <span>
                <input type="text" v-model="ipaddr" :style="{ 'border-color': ipaddr !== '' && !reg_ip.test(ipaddr) ? 'red' : '' }">
        ...
        <div class="diagonose-item">
            ...
            <span>
                <input type="text" v-model="count" :style="{ 'border-color': count > 50 || count < 1 ? 'red' : '' }">
                <span>(1-50)</span>
            </span>
        </div>
        <div class="diagonose-item">
            ...
            <span>
                <input type="text" v-model="pktsize" :style="{ 'border-color': pktsize > 1472 || pktsize < 4 ? 'red' : '' }">
                <span>(4-1472 Bytes)</span>
            </span>
        </div>
        <div class="diagonose-item">
            <a href="javascript:void(0);" @click="ping_start">{{ lanMap['start'] }}</a>
        </div>
        <div class="diagonose-item">
    ...
    export default {
                ...
                this.$http.get(this.change_url.inbound).then(res=>{
                    if(res.data.code == 1){
                        ...
                        if(res.data.data){
                         ...
                         this.interface = this.interface_data[0].interface;
                    }else{
                        this.inbound = {};
                    }
                }).catch(err=>{
        ...
        methods: {
            ping_start(){
                ...
                this.$http.post('/diagnose?form=ping',post_params).then(res=>{
                    if(res.data.code === 1){
                        this.result = '';
                        this.ping_result();
            ...
            ping_result(){
                this.$http.get('/diagnose?form=ping').then(res=>{
                    if(res.data.code === 1){
