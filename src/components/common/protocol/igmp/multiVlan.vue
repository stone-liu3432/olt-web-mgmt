<template>
    <div class="multi-vlan">
        ...
        <div v-if="mv_info.data && mv_info.data.length > 0" class="multi-vlan-content">
            <div>
                ...
                <select v-model.number="mvlan">
                    <option
                        :value="item.mvlan"
                        v-for="(item,index) in mv_info.data"
                        :key="index"
                    >{{ item.mvlan }}</option>
                </select>
            ...
            <div>
                ...
                <span>
                    <template
                        v-if="router_plist.router_portlist"
                    >{{ router_plist.router_portlist | parsePortList }}</template>
            ...
            <div class="program">
                ...
                <span>
                    ...
                    <span v-for="(item,index) in program.data" :key="index">
                        <span>{{ item.program_s === item.program_e ? item.program_s : item.program_s + ' - ' + item.program_e }}</span>
                        <span :title="item.program_desc">{{ item.program_desc }}</span>
                        <span>
                            <a
                                href="javascript:void(0);"
                                @click="open_cfg_pro_desc(item)"
                            >{{ lanMap['config'] }}</a>
                            <a
                                href="javascript:void(0);"
                                @click="open_del_single_program(item)"
                            >{{ lanMap['delete'] }}</a>
                        </span>
                    </span>
                </span>
        ...
        <div class="modal-dialog" v-if="is_cfg_mv">
            <div class="cover"></div>
            <div
                class="multi-vlan-set"
                :style="{ 'height' : mark_type === 0 && mark_mode === 1 ? '420px' : '' }"
            >
                <div>
                    ...
                    <div>
                        ...
                        <select v-model.number="mark_type" :disabled="mark_mode === 2">
                            ...
                            <option
                                value="2"
                                v-if="mark_mode === 2"
                            >{{ lanMap['mc_unknown_policy'] }}</option>
                        </select>
                    </div>
                    ...
                    <div
                        v-if="mark_type === 0"
                        class="multi-tips"
                    >{{ lanMap['program_range_tips'] }}</div>
                    <div
                        v-else-if="mark_type === 1"
                        class="multi-tips"
                    >{{ lanMap['port_list_tips'] }}</div>
                    <div v-else></div>
                    <div class="program-desc" v-if="mark_type === 0 && mark_mode === 1">
                        <span>{{ lanMap['program_desc'] }}</span>
                        <textarea
                            spellcheck="false"
                            v-model="program_desc"
                            placeholder="0-64 characters"
                            :style="{ 'border-color': program_desc.length > 64 ? 'red' : '' }"
                        ></textarea>
                    </div>
                    <div>
        ...
        <div class="modal-dialog" v-if="is_create_mvlan">
            <div class="cover"></div>
            <div
                class="create-mvlan-modal"
                :style="{ 'height': is_set_mvdesc ? '275px' : '165px' }"
            >
                <div>
                    <h3
                        v-if="!is_set_mvdesc"
                        class="modal-header"
                    >{{ lanMap['create'] + lanMap['mvlan']}}</h3>
                    <h3
                        v-else
                        class="modal-header"
                    >{{ lanMap['config'] + lanMap['mvlan'] + lanMap['desc'] }}</h3>
                    <div>
                        <span>{{ lanMap['vlan_id'] }}</span>
                        <input
                            type="text"
                            v-model.number="create_mvlan"
                            v-focus
                            v-if="!is_set_mvdesc"
                    ...
                    <div v-if="is_set_mvdesc">
                        ...
                        <textarea
                            ...
                            v-model="mvlan_desc"
                            placeholder="0-64 characters"
                            :style="{ 'border-color': mvlan_desc.length > 64 ? 'red' : '' }"
                        ></textarea>
                    </div>
                    <div v-if="!is_set_mvdesc">
                        <a
                            href="javascript:void(0);"
                            @click="submit_create_mvlan"
                        >{{ lanMap['apply'] }}</a>
                        <a
                            href="javascript:void(0);"
                            @click="close_create_mvlan"
                        >{{ lanMap['cancel'] }}</a>
                    </div>
                    <div v-else>
                        <a
                            href="javascript:void(0);"
                            @click="submit_set_mvdesc"
                        >{{ lanMap['apply'] }}</a>
                        <a
                            href="javascript:void(0);"
                            @click="close_create_mvlan"
...
export default {
    ...
    methods: {
                ...
                .then(res => {
                    if (res.data.code === 1) {
                        ...
                        if (res.data.data && res.data.data.length > 0) {
                            ...
                            for (
                                var i = 0, len = res.data.data.length;
                                i < len;
                                i++
                            ) {
                                //  mvlan存在  且在列表中时
        ...
        submit_cfg() {
            ...
            if (this.mark_mode === 2) {
                if (this.mark_type === 2) {
                    //  修改未知多播策略
                    if (
                        this.mc_unknown_info.data.mc_unknown_policy ===
        ...
        add_program() {
            ...
            this.$http
                .post("/switch_igmp?form=program", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        add_router_port() {
            var post_param = {
                ...
                param: {
                    mvlan: this.mvlan,
                    router_portlist: this.router_portlist.replace(
                        /\d/g,
                        n => new String(Number(n) + this.system.data.ponports)
                    )
                }
            };
            this.$http
                .post("/switch_igmp?form=router_port", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        modify_unknow_policy() {
            ...
            this.$http
                .post("/switch_igmp?form=mc_unknown", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        result_delmvlan(bool) {
            ...
            this.$http
                .post("/switch_igmp?form=multicast_vlan", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        submit_create_mvlan() {
            if (
                this.create_mvlan < 1 ||
                this.create_mvlan > 4094 ||
                isNaN(this.create_mvlan)
            ) {
                this.$message({
                    type: "error",
        ...
        submit_set_mvdesc() {
            ...
            this.$http
                .post("/switch_igmp?form=multicast_vlan_desc", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
    ...
    watch: {
        ...
        mark_mode() {
            if (this.mark_mode === 2) {
                ...
                if (
                    this.mark_type === 1 &&
                    (!this.router_plist || !this.router_plist.router_portlist)
                ) {
                    this.mark_mode = 1;
                    return;
                }
                this.router_portlist = this.router_plist.router_portlist.replace(
                    /\d/g,
                    n => new String(Number(n) - this.system.data.ponports)
                );
