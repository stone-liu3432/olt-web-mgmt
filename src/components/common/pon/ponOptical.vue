<template>
    <div class="pon-optical">
        <div class="pon-optical-title">
            ...
            <div v-if="is_show_onu && onu_list.data">
                <span>{{ lanMap['onu_id'] }}</span>
                <select v-model.number="onuid">
                    <option v-for="(item,index) in onu_list.data" :key="index" :value="item">{{ 'ONU'+ portid + '/' + item }}</option>
                </select>
            </div>
        ...
        <hr>
        <tabBar :tab="['pon_optical','onu_optical_diagnose']"  @togglePage="select_page" v-if="onu_list.data && onu_list.data.length > 0"></tabBar>
        <!-- RSSI -->
        <a href="javascript:void(0);" class="rt" @click="reload" v-if="show_index === 'pon_optical' && pon_optical.data && onu_list.data && onu_list.data.length > 0">
            {{ lanMap['refresh'] }}
        </a>
        <p v-if="show_index === 'pon_optical' && pon_optical.data && onu_list.data && onu_list.data.length > 0">{{ lanMap['pon_optical_tips'] }}</p>
        <ul class="rssi" v-if="show_index === 'pon_optical' && pon_optical.data && onu_list.data && onu_list.data.length > 0">
        ...
        <!-- 所有ONU的光模块诊断信息 -->
        <ul class="onu-optical" v-if="show_index === 'onu_optical_diagnose' && all_onu_optical.data && all_onu_optical.data.length > 0 && onu_list.data && onu_list.data.length > 0">
            <li class="bg-title">
                <span>{{ lanMap['onu_id'] }}</span>
            ...
            <li v-for="(item,key) in all_onu_optical.data" :key="key">
                <span :title="item.onu_name">{{ item.onu_name ? item.onu_name : 'PON0' + item.port_id + '/' + item.onu_id }}</span>
        ...
        <p class="error-tips" v-if="show_index === 'onu_optical_diagnose' && !all_onu_optical.data">{{ lanMap['onu_optical_tips'] }}</p>
        <div class="error-tips" v-if="timeout_tips">{{ lanMap['get_data_fail'] }}</div>
...
export default {
    name: 'ponOptical',
    computed: mapState(['lanMap','port_name','change_url','onu_list']),
    data(){
        return {
            portid: 0,
            onuid: 0,
    ...
    methods: {
        ...
        //  RSSI
        getData(){
            this.$http.get('/ponmgmt?form=optical_pon',{ params: { 'port_id': this.portid, 'onu_id': this.onuid } }).then(res=>{
                if(res.data.code === 1){
        ...
        get_all_optical(){
            ...
            this.is_load = true;
            this.$http.get('/ponmgmt?form=optical_onu',{ params: { 'port_id': this.portid } }).then(res=>{
                this.is_load = false;
                if(res.data.code === 1){
