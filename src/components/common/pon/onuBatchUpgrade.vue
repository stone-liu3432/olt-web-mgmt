<template>
    <div class="onu-batch-upgrade">
        ...
        <div>
            ...
            <div class="onu-type-mgmt">
                ...
                <div>
                    ...
                    <a href="javascript:void(0);" @click="open_bind_modal">{{ lanMap['onu_bind_upgrade_type'] }}</a>
                </div>
                <div>
                    <span>{{ lanMap['current_upgrade_type'] + '：' }}</span>
                    <span v-if="upgrade_type.data && upgrade_type.data.length > 0">{{ upgrade_type.data.toString() }}</span>
            ...
            <div class="onu-type-mgmt">
                ...
                <div class="onu-upgrade">
                    ...
                    <form class="upload-form">
                        <input type="file" id="onu-upgrade-file" class="hide" @change="changeFile('onu-upgrade-file','onu-upgrade-filename')" />
                        <span class="updateFile" id="onu-upgrade-filename">{{ lanMap['file_click'] }}</span>
                    </form>
                </div>
                <div class="onu-upgrade">
                    ...
                    <select v-model="param_otp" v-if="upgrade_type.data">
                        ...
                        <option :value="item" v-for="(item,index) in upgrade_type.data" :key="index">{{ item }}</option>
                    </select>
                    <span v-else>{{ lanMap['no_upgrade_type'] }}</span>
                </div>
                <div class="onu-upgrade">
        ...
        <div class="modal-dialog" v-if="show_otp_modal">
            ...
            <div class="onu-type-modal">
                ...
                <div>
                    ...
                    <select v-if="!show_add_modal && otp_list.length && otp_list.length > 0" v-model="del_otp">
                        <option value="">{{ lanMap['choose'] }}</option>
                        <option :value="item" v-for="(item,index) in upgrade_type.data" :key="index">{{ item }}</option>
                    </select>
                    <span v-if="!show_add_modal && !upgrade_type.data">{{ lanMap['no_upgrade_type'] }}</span>
                </div>
                ...
                <div>
                    ...
                    <div>
                        ...
                        <select v-if="otp_list.length && otp_list.length > 0" v-model="bind_otp">
                            <option value="">{{ lanMap['choose'] }}</option>
                            <option :value="item" v-for="(item,index) in otp_list" :key="index">{{ item }}</option>
                        </select>
                        <span v-else>{{ lanMap['no_upgrade_type'] }}</span>
                    </div>
                    <div>
        ...
        <confirm v-if="show_cfm_modal" @choose="result_cfm_upgrade"></confirm>
    </div>
</template>
...
export default {
    ...
    methods: {
        get_upgrade_type(){
            this.$http.get('/onu_upgrade?form=upgrade_type').then(res=>{
                if(res.data.code === 1){
        ...
        add_otp_submit(){
            ...
            this.$http.post('/onu_upgrade?form=upgrade_type',post_params).then(res=>{
                if(res.data.code === 1){
                    this.$message({
                        type: res.data.type,
        ...
        open_cfm_upgrade(){
            ...
            this.show_cfm_modal = true;
        },
        //  confirm 选择结果处理函数
        result_cfm_upgrade(bool){
            if(bool){
                this.upload_file();
            }
            this.show_cfm_modal = false;
        },
        //  上传文件
        upload_file(){
            ...
            this.$http.post('/onu_upload?type=' + this.param_otp, formData,{
        ...
        get_onu_baseinfo(){
            this.$http.get(this.change_url.get_onu_baseinfo).then(res=>{
                if(res.data.code === 1){
        ...
        submit_upgrade(){
            ...
            this.$http.post('/onu_upgrade?form=bind_upgrade_type',post_params).then(res=>{
                if(res.data.code === 1){
                    this.$message({
                        type: res.data.type,
                        text: this.lanMap['setting_ok']
