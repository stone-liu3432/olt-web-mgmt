<template>
    <div>
        ...
        <onuPortVlan
            v-if="show_page === 'mvlan' && onu_list.data"
            :portid="portid"
            :onuid="onuid"
            ref="opvlan"
        ></onuPortVlan>
        <div class="modal-dialog" v-if="modal_index === 1 || modal_index === 2">
            ...
            <div class="onu-multi-cfg">
                <h4
                    v-if="modal_index === 1"
                    class="modal-header"
                >{{ lanMap['onu_multicast'] + ' ' + lanMap['config'] }}</h4>
                <h4
                    v-if="modal_index === 2"
                    class="modal-header"
                >{{ lanMap['fast_leave'] + ' ' + lanMap['config'] }}</h4>
                ...
                <div>
                    <a
                        ...
                        v-if="modal_index === 1"
                        @click="submit_mc_mode"
                    >{{ lanMap['apply'] }}</a>
                    <a
                        href="javascript:void(0);"
                        v-if="modal_index === 2"
                        @click="submit_fast_leave"
                    >{{ lanMap['apply'] }}</a>
...
export default {
    ...
    methods: {
        ...
        submit_mc_mode() {
            ...
            this.$http
                .post("/onumgmt?form=mc_mode", post_param)
                .then((res) => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        submit_fast_leave() {
            ...
            this.$http
                .post("/onumgmt?form=mc_fast_leave", post_param)
                .then((res) => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
