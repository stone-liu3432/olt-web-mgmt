<template>
    <div>
        ...
        <div>
            ...
            <a href="javascript:void(0);" @click="deleteAll">{{ lanMap['delete_all'] }}</a>
        </div>
        <div
            v-for="(item,index) in timeRange.data"
            :key="index"
        ...
        >
            ...
            <ul>
                ...
                <li>
                    <a
                        ...
                        @click="showTimeRangeDetail(item)"
            ...
            <div v-if="showTimeRange === item.name">
                <div
                    v-for="(_item,_index) in item.absolute"
                    :key="_index + _item.stime"
                    v-if="item.absolute"
                    class="absolute-item"
                >
                ...
                <div
                    v-for="(_item,_index) in item.relative"
                    :key="_index + _item.sday"
                    v-if="item.relative"
                    class="relative-item"
                >
        ...
        <div class="modal-dialog" v-if="!!modalFlag">
            <div class="cover"></div>
            <div
                :style="{ 'height': modalFlag === 'delete' ? '275px' : type === 2 ? '355px' : '315px' }"
            >
...
export default {
    ...
    methods: {
        ...
        submitAdd() {
            ...
            if (!this.regTime.test(this.stime)) {
                this.$message({
                    type: "error",
                    text:
                        this.lanMap["param_error"] + ": " + this.lanMap["stime"]
                });
                return;
            }
            if (!this.regTime.test(this.etime)) {
                this.$message({
                    type: "error",
                    text:
                        this.lanMap["param_error"] + ": " + this.lanMap["etime"]
                });
                return;
            }
            var post_data, url;
        ...
        deleteCallback(name, time, day) {
            ...
            this.$http
                .post("/switch_acl?form=time_range", post_param)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
        ...
        changePage(index) {
            ...
            end > this.timeRange.data.length &&
                (end = this.timeRange.data.length);
            this.timeRangeShow = this.timeRange.data.slice(
                limit * (index - 1),
                limit * index
            );
        }
