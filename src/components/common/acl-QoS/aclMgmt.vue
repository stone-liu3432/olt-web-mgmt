    ...
    <div>
        ...
        <div v-for="(item,index) in acl_show" :key="index" class="acl-item" v-if="acl_show.length">
            ...
            <ul>
                ...
                <li
                    v-if="item.rule && item.rule.length"
                >{{ lanMap['rule_count'] }} : {{ item.rule.length }}</li>
                <li v-else></li>
                <li>
                    <a
                        href="javascript:void(0);"
                        @click="showDetail(item.acl_id)"
                    >{{ lanMap['show_rule'] }}</a>
                    <a
                        href="javascript:void(0);"
                        @click="openRuleCfgModal(item)"
                        v-if="item.rule && item.rule.length > 0"
                    >{{ lanMap['config'] }}</a>
                    <a
                        href="javascript:void(0);"
                        @click="openRuleAddModal(item)"
                    >{{ lanMap['add'] }} Rule</a>
                    <a
                        href="javascript:void(0);"
                        @click="ARPLRule(item)"
                        v-if="show_acl_id === item.acl_id && item.rule && item.rule.length"
                    >
            ...
            <div
                v-for="(_item,_index) in item.rule"
                :key="_index"
                class="acl-detail"
                v-if="show_acl_id === item.acl_id && item.rule && item.rule.length"
            >
                <div class="column-header">
                    ...
                    <div v-else></div>
                    <div>{{ 'RULE ID' }}:{{ _item.rule_id }}</div>
                </div>
                <div class="acl-rule-detail" ref="acl_rule_detail">
                    <span
                        v-if="_item.action"
                    >{{ lanMap['action'] }}&nbsp;:&nbsp;{{ _item.action === 1 ? 'deny' : 'permit' }}</span>
                    <span
        ...
        <div class="modal-dialog" v-if="isAclMgmt">
            <div class="cover"></div>
            <div
                class="modal-centent acl-mgmt-modal"
                :style="{ 'height': isAddAcl ? '210px' : '' }"
            >
                ...
                <div>
                    <a
                        ...
                        @click="addACL"
                    >{{ lanMap['apply'] }}</a>
                    <a href="javascript:void(0);" v-else @click="deleteACL">{{ lanMap['apply'] }}</a>
                    <a href="javascript:void(0);" @click="closeAclMgmt">{{ lanMap['cancel'] }}</a>
                </div>
                ...
                <div class="acl-rule-config">
                    ...
                    <div>
                        ...
                        <select v-if="isCfgRule" v-model.number="rule_id">
                            <option
                                :value="item.rule_id"
                                v-for="(item,index) in cache_data.rule"
                                :key="index"
                            >{{ item.rule_id }}</option>
                        </select>
                        <input
                    ...
                    <div v-if="acl_rule_type === 2">
                        ...
                        <input
                            ...
                            placeholder="icmp/udp/tcp/ip/ipinip"
                    ...
                    <div v-if="acl_rule_type === 2">
                        ...
                        <input
                            type="text"
                            v-model="src_port"
                            :disabled="protocol.toString().toLowerCase() === 'icmp'"
                    ...
                    <div>
                        ...
                        <select v-model="timerange">
                            ...
                            <option
                                :value="item.name"
                                v-for="(item, index) in time_range.data"
                                :key="index"
                            >{{ item.name }}</option>
                        </select>
                    </div>
...
export default {
    ...
    methods: {
        ...
        getTimeRange() {
            this.$http
                .get(this.change_url.get_timerange)
                .then(res => {
                    if (res.data.code === 1) {
        ...
        openDeleteModal(acl_id, rule_id) {
            this.$confirm()
                .then(_ => {
                    this.deleteModalResult(acl_id, rule_id);
                })
                .catch(_ => {});
        },
        deleteModalResult(acl_id, rule_id) {
            var post_data = {
                method: "delete",
                param: {
                    acl_id,
            ...
            this.$http
                .post("/switch_acl?form=rule", post_data)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: res.data.type,
                        ...
                        this.acl_all.data.forEach((item, index, arr) => {
                            if (item.acl_id === acl_id) {
                                item.rule.forEach((_item, _index, _arr) => {
                                    if (_item.rule_id === rule_id) {
        ...
        addACL() {
            ...
            this.$http
                .post("/switch_acl?form=acl", post_data)
                .then(res => {
                    if (res.data.code === 1) {
                        this.$message({
                            type: "success",
        ...
        deleteACL() {
            if (
                this.IDRangeError(this.acl_id) ||
                this.IDRangeError(this.acl_id_e)
            ) {
                this.$message({
                    type: "error",
                    text: this.lanMap["acl_id_range_error"]
        ...
        cfgBasicRule() {
            ...
            n++;
            if (
                !this.testIP(this.src_ipmask) &&
                this.src_ipaddr !== "0.0.0.0"
            ) {
                this.$message({
                    type: "error",
                    text:
        ...
        cfgAdvancedRule() {
            var flags = 0x1,
                n = 0;
            if (
                (this.protocol.toString().toLowerCase() !== "icmp" &&
                    this.protocol.toString().toLowerCase() !== "udp" &&
                    this.protocol.toString().toLowerCase() !== "tcp" &&
                    this.protocol.toString().toLowerCase() !== "ip" &&
            ...
            if (this.dscp < 0 || this.dscp > 255 || isNaN(this.dscp)) {
                this.$message({
                    type: "error",
                    text:
                        this.lanMap["param_error"] + ": " + this.lanMap["dscp"]
                });
                return;
            }
            if (this.dscp !== "") {
        ...
        cfgLinkRule() {
            ...
            if (
                this.vlan_id !== "" &&
                (this.vlan_id < 1 || this.vlan_id > 4094 || isNaN(this.vlan_id))
            ) {
                this.$message({
                    type: "error",
                    text:
            ...
            if (
                this.inner_vlan_id !== "" &&
                (this.inner_vlan_id < 1 ||
                    this.inner_vlan_id > 4094 ||
                    isNaN(this.inner_vlan_id))
            ) {
                this.$message({
                    type: "error",
                    text:
        ...
        swapRulePriority(node, index, flag) {
            ...
            if (flag === 1) {
                var _node = node.rule[index],
                    next = node.rule[index + 1];
                this.$set(node.rule, index, next);
                this.$set(node.rule, index + 1, _node);
            }
            if (flag === -1) {
                var _node = node.rule[index],
                    prev = node.rule[index - 1];
                this.$set(node.rule, index, prev);
                this.$set(node.rule, index - 1, _node);
            }
        },
        changePage(index) {
            let start = limit * (index - 1),
                end = limit * index;
        ...
        ARPLRule(node) {
            if (this.isARPL) {
                ...
                this.$http
                    .post("/switch_acl?form=rulepri", post_data)
                    .then(res => {
                        if (res.data.code === 1) {
                            this.$message({
                                type: "success",
